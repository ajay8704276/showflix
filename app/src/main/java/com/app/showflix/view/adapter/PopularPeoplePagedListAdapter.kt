...
import com.app.showflix.utils.Constants.POSTER_BASE_URL
import com.app.showflix.utils.NetworkState
import com.app.showflix.view.activities.people.PopularPeopleDetailActivity
import com.bumptech.glide.Glide
import kotlinx.android.synthetic.main.network_state_item.view.*
import kotlinx.android.synthetic.main.people_list_item.view.*

class PopularPeoplePagedListAdapter(private val context: Context) :
    PagedListAdapter<Results, RecyclerView.ViewHolder>(PeopleDiffCallback()) {
    ...
    class PeopleDiffCallback : DiffUtil.ItemCallback<Results>() {
    ...
    class PeopleItemViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        fun bind(people: Results?, context: Context) {
            ...
            itemView.gender.text = people?.gender.toString()

            val moviePosterURL = POSTER_BASE_URL + people?.profile_path
            Glide.with(itemView.context)
                .load(moviePosterURL)
                .into(itemView.profile_path)

            itemView.setOnClickListener {
                Toast.makeText(context, "Item clicked", Toast.LENGTH_LONG).show()
                val intent = Intent(context, PopularPeopleDetailActivity::class.java)
